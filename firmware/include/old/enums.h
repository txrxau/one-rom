...
STATIC_ASSERT(sizeof(sdrr_rom_type_t) == 1, "sdrr_rom_type_t must be 1 byte");
...
STATIC_ASSERT(sizeof(sdrr_cs_state_t) == 1, "sdrr_cs_state_t must be 1 byte");

typedef enum {
    BIT_MODE_8  = 0x01,
    BIT_MODE_16 = 0x02,
} bit_modes_t;
STATIC_ASSERT(sizeof(bit_modes_t) == 1, "bit_modes_t must be 1 byte");

typedef enum {
    FIRE_SERVE_CPU = 0x00,
    FIRE_SERVE_PIO = 0x01,
} fire_serve_modes_t;
STATIC_ASSERT(sizeof(fire_serve_modes_t) == 1, "fire_serve_modes_t must be 1 byte");
...
STATIC_ASSERT(sizeof(mcu_line_t) == 2, "mcu_line_t must be 2 bytes");
...
typedef enum {
    ...
    STORAGE_G = 0x06,
    STORAGE_2MB = 0x07,
    MCU_STORAGE_FORCE_UINT16 = 0xFFFF,
} mcu_storage_t;
STATIC_ASSERT(sizeof(mcu_storage_t) == 2, "mcu_storage_t must be 2 bytes");
...
STATIC_ASSERT(sizeof(fire_vreg_t) == 1, "fire_vreg_t must be 1 byte");
...
STATIC_ASSERT(sizeof(limp_mode_pattern_t) == 1, "limp_mode_pattern_t should be 1 byte");
...
// Matches hardware register values for INOVER field of GPIOx_CTRL registers,
// used for overriding GPIO inputs.
typedef enum __attribute__((packed)) {
    GPIO_OVER_NORMAL = 0,
    GPIO_OVER_INVERT = 1,
    GPIO_OVER_LOW = 2,
...
// Type of ROM slot.  Doesn't hold ROM type itself - holds higher level
// information about the slot.
typedef enum __attribute__((packed)) {
    ROM_SLOT_TYPE_PLUGIN_SYSTEM = 0,
    ROM_SLOT_TYPE_PLUGIN_USER = 1,
    ROM_SLOT_TYPE_PLUGIN_PIO = 2,
    ROM_SLOT_TYPE_SINGLE_ROM = 3,
    ROM_SLOT_TYPE_MULTI_ROM = 4,
    ROM_SLOT_TYPE_BANKED_ROM = 5,
    ROM_SLOT_TYPE_SINGLE_RAM = 6,
    NUM_ROM_SLOT_TYPES
} rom_slot_type_t;
